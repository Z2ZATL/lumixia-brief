...
Expose a stateless Streamable HTTP MCP endpoint at `/api/mcp`. Authenticate it with Supabase OAuth 2.1 and require the existing Google identity, verified TOTP/AAL2, authenticated audience and role, UUID subject, explicit OAuth client identifier, owner RLS, and write approval in Codex.
