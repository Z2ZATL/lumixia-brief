...
- Importing the live check exposed that the migration job's previous job-level release-flag condition produced `skipped`, which is not a fail-closed promotion signal. The job now runs for every `main` push, waits on the protected `production` environment, checks `PRODUCTION_RELEASE_ENABLED` only after owner approval, and then applies forward-only migrations. A regression test locks the ordering and prevents the release flag from returning to the job-level condition.
...
- The post-configuration clean gate passed formatting, zero-warning ESLint, strict TypeScript, Knip, the CSS consumer audit, 92 unit/API tests, 19 UI tests, 11 local Supabase Auth/RLS integration tests, 8 desktop/mobile Playwright tests, the Production build and bundle ceiling, and both dependency audits. Backend coverage before the release-workflow regression was 95.13% lines and 84.31% branches. A Linux/amd64 Docker build repeated typecheck, all 92 tests, and the build; Trivy found zero critical vulnerabilities, and Gitleaks scanned 81 commits with zero findings. The temporary portability image was removed after verification.
- The first CI execution of the workflow regression failed only in the portability build because `.dockerignore` excluded the workflow file that the new test reads. The build context now includes only `.github/workflows/ci.yml`, not the rest of `.github`; a second assertion protects that narrow exception. A local Linux/amd64 build reproduced the CI path and passed all 92 tests plus the Production build before its temporary image was removed.
- Evidence commits: `978ca04`, `f242be9`, `6219560`, `edd5f62`, `4bccf9e`; [PR #30](https://github.com/Z2ZATL/lumixia-brief/pull/30). [Required CI run 29632600042](https://github.com/Z2ZATL/lumixia-brief/actions/runs/29632600042) passed every quality, Auth/RLS, browser, container/SBOM, Trivy, and secret-scan job before the release-gate regression commit. [CI run 29632883051](https://github.com/Z2ZATL/lumixia-brief/actions/runs/29632883051) provided the portability regression evidence.
